...
# Milestone 4 — Regression comparison (v1)

**Goal:** Compare prompt versions via session outputs.

### Deliverables

**Regression runner**

- `workbench/regress.py`
  - runs suite twice, creating two session folders:
    - `session_id_A` (prompt v1)
    - `session_id_B` (prompt v2)
  - writes:
    - `reports/sessions/<session_id_A>/results.ndjson`
    - `reports/sessions/<session_id_B>/results.ndjson`
    - `reports/regress/<regress_id>/diff.md` (overwrites)
  - optionally exports per-session CSV snapshots

**Prompt versioning**

- `prompts/v1/...` and `prompts/v2/...` or config mapping
...
- command produces two session logs + a human-readable diff with trace pointers

---

# Milestone 5 — Optional v1.1

### Option A: Metamorphic transform

- Apply `rent * 1.10` to subset of tasks during suite run
- metric: `metamorphic_flip_rate`
- logged per session

### Option B: Tool calling / MCP (upgrade)

- expose `validate_scenario` + `run_eval` as MCP/tools
- agent uses tool calls
- **now** add taxonomy:
  - `NO_TOOL_USE` (when required)
  - `TOOL_CALL_HALLUCINATION` (claims results not in trace)
- add `model_tool_calls` metric (non-zero)
